...
test("catalog /v1/models: exact provider-grouped order (blocks === distinct owned_by)", async () => {
  ...
  await modelsDb.replaceSyncedAvailableModelsForConnection("openai", conn1.id, [
    { id: "gpt-4", name: "GPT-4" },
    { id: "gpt-3.5-turbo", name: "GPT-3.5 Turbo" },
  ]);
  await modelsDb.replaceSyncedAvailableModelsForConnection("anthropic", conn2.id, [
    { id: "claude-3-opus", name: "Claude 3 Opus" },
  ]);
  await modelsDb.replaceSyncedAvailableModelsForConnection("opencode", conn3.id, [
...
test("catalog /v1/models: combo block appears first", async () => {
  ...
  await modelsDb.replaceSyncedAvailableModelsForConnection("openai", conn.id, [
    { id: "gpt-4", name: "GPT-4" },
  ]);
